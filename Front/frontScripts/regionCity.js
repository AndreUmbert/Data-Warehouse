...
function showRegions(regions) {
  for (let region of regions) {
    ...
    for (let country of region.countries.values()) {
      ...
      countryNameUl.appendChild(document.createTextNode(country.countryName));
      ...
      editCountry.setAttribute("countryName", country.countryName);
      //Delete Country Button:
      const deleteCountry = document.createElement("div");
      deleteCountry.setAttribute("class", "deleteCountry");
      deleteCountry.setAttribute("countryName", country.countryName);
      ...
      for (let city of country.cities.values()) {
        ...
        cityEntry.appendChild(document.createTextNode(city.cityName));
        ...
        editCity.setAttribute("cityName", city.cityName);
        // Delete City Button
        ...
        deleteCity.setAttribute("cityName", city.cityName);
...
function createCountry(element) {
  ...
  createButton.addEventListener("click", async () => {
    ...
    let regPost = await axios.post(
      "http://localhost:3000/country/create",
      {
        countryName: inputCountry.value,
        regionId: reg.data[element.getAttribute("countrynumber")].id,
      },
      config
    );
    location.reload();
  ...
  createButton.addEventListener("click", async () => {
    ...
    let countPost = await axios.post(
      "http://localhost:3000/city/create",
      {
        ...
        countryId: element.getAttribute("countryId"),
      },
      config
    );
    location.reload();
...
function deleteCity(element) {
  ...
  axios.delete(`http://localhost:3000/city/delete/${city}`, config);
  location.reload();
}
...
function editCity(element) {
  ...
  editButton.addEventListener("click", async (element) => {
    let cityEdit = await axios.put(
      `http://localhost:3000/city/update/${cityName}`,
      { newName: editCity.value },
      config
    );
    location.reload();
  });
...
function editCountry(element) {
  ...
  editButton.addEventListener("click", async (element) => {
    let countryEdit = await axios.put(
      `http://localhost:3000/country/update/${countryName}`,
      { newName: editCountry.value },
      config
    );
    location.reload();
  });
