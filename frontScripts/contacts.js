...
contactsSearchBarTextInput.addEventListener("input", async (e) => {
    ...
    for (let index = 0; index < contacts.data.length; index++) {
        allContactsArray.push(contacts.data[index].contactName)
        if (contacts.data[index].contactName.includes(value)) {
            searchContacts.push(contacts.data[index].contactName)
            console.log(contacts.data[index].contactName);
        }
    }
